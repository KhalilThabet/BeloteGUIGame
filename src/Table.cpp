...
vector<Carte>& Table:: setCardsOnTable(Carte cart)
{
	if(CardsOnTable.size()<4)
	{CardsOnTable.push_back(cart);}
}
...
int Table:: joueur_gagnant(string atou,int b)   //retourne l'indice du joueur gagant dans le le vecteur joueurs
{
    ...
    {
        if(CardsOnTable[i]==compareTableCards(string atou))
...
{   int somme=0;
    for(int i=0;i<CardsOnTable.size();i++)
    {
        if(CardsOnTable.couleur==at)
        {
            somme+=CardsOnTable.val_atout;
        }
        else
        {
            somme+=CardsOnTable.val_hors_atout;
        }
    }
    return somme;
}
