...
public:
    ...
    vector<Carte>& setCardsOnTable(Carte);
    void melange();                  //Melange des cartes du vecteur AllCards
    Carte compareTableCards(string);
    int joueur_gagnant(string atou,int b);   //retourne l'indice du joueur gagant dans le le vecteur joueurs
    int somme_score(string at);   //retourne la somme des 4 cartes sur table 
    void score(string atout,int b);
